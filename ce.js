// bot.js - Modified for CE Server Only
...
(async () => {
    ...
    await context.addInitScript(() => {
        ...
        window.processQueue = async () => {
            ...
            try {
                // --- MODIFIED BEHAVIOR: Wait 500-650ms, then send /claim .3 ---
                console.log(`[Bot Script] 🚀 Preparing to send message to ${serverName} server...`);
                
                // Random delay between 500ms and 650ms before sending the message
                const delay = 500 + Math.random() * 150;
                console.log(`[Bot Script] ⏱️ Waiting ${Math.round(delay)}ms before sending message...`);
                await new Promise(resolve => setTimeout(resolve, delay));
                
                const payload = { content: '/claim .3', nonce: window.generateNonce(), replies: [] };
