...
(async () => {
    ...
    await context.addInitScript(() => {
        ...
        window.processQueue = async () => {
            ...
            try {
                ...
                console.log(`[Bot Script] ⏳ Waiting ${initialDelay}ms before sending ticket number...`);
                ...
                if (!response.ok) {
                    ...
                    if (responseData._id) {
                        ...
                        console.log(`[Bot Script] ⏳ Waiting ${secondDelay}ms before sending second message...`);
                        
                        await new Promise(resolve => setTimeout(resolve, secondDelay));
                        
                        const secondPayload = { content: ".3", nonce: window.generateNonce(), replies: [] };
                        console.log(`[Bot Script] 🚀 Sending second message (.3) via Fetch: ${JSON.stringify(secondPayload)}`);
                        ...
                        console.log(`[Bot Script] 📡 Second message API Response received. Status: ${secondResponse.status} ${secondResponse.statusText}`);
                        
                        if (!secondResponse.ok) {
                            const secondErrorText = await secondResponse.text();
                            console.log(`[Bot Script] ❌ Second message API Error Response: ${secondErrorText}`);
                        } else {
                            const secondResponseData = await secondResponse.json();
                            if (secondResponseData._id) {
                                console.log(`[Bot Script] ✅ Second message (.3) sent successfully with ID: ${secondResponseData._id}`);
                            } else {
                                console.log(`[Bot Script] ⚠️ Second message appeared to send but no message ID was returned. Response: ${JSON.stringify(secondResponseData)}`);
                            }
                        }
                    } else {
    ...
    console.log("🚀 Bot is now running for the target server. It will wait before sending the ticket number, then send '.3' with a random delay.");
