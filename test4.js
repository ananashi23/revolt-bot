...
console.log("--- Starting Enhanced Revolt Bot (Rate Limited, Memory Optimized, Server Delays & Latency Measurement) ---");
...
class RateLimiter {
    ...
    async execute(task, priority = false) {
        return new Promise((resolve, reject) => {
            ...
            if (priority) {
                // Insert priority items at the front, but after other priority items
...
(async () => {
    ...
    const showStatus = async () => {
        ...
        if (status.latencyStats.count > 0) {
            console.log("\n--- Latency Statistics ---");
            console.log(`Measurements: ${status.latencyStats.count}`);
            console.log(`Network + API Time: Avg: ${status.latencyStats.avgNetworkTime.toFixed(2)}ms, Min: ${status.latencyStats.minNetworkTime.toFixed(2)}ms, Max: ${status.latencyStats.maxNetworkTime.toFixed(2)}ms`);
            console.log(`Total Time: Avg: ${status.latencyStats.avgTotalTime.toFixed(2)}ms, Min: ${status.latencyStats.minTotalTime.toFixed(2)}ms, Max: ${status.latencyStats.maxTotalTime.toFixed(2)}ms`);
            console.log(`Processing Time: Avg: ${status.latencyStats.avgProcessingTime.toFixed(2)}ms`);
        } else {
            console.log("\n--- Latency Statistics ---");
    ...
    // Add server-specific latency statistics command
    const showServerStatus = async (serverName) => {
        const serverStats = await page.evaluate((server) => {
            return window.latencyTracker.getServerStats(server);
        }, serverName);
        
        console.log(`\n--- ${serverName} Server Latency Statistics ---`);
        if (serverStats.count > 0) {
            console.log(`Measurements: ${serverStats.count}`);
            console.log(`Network + API Time: Avg: ${serverStats.avgNetworkTime.toFixed(2)}ms, Min: ${serverStats.minNetworkTime.toFixed(2)}ms, Max: ${serverStats.maxNetworkTime.toFixed(2)}ms`);
            console.log(`Total Time: Avg: ${serverStats.avgTotalTime.toFixed(2)}ms, Min: ${serverStats.minTotalTime.toFixed(2)}ms, Max: ${serverStats.maxTotalTime.toFixed(2)}ms`);
            console.log(`Processing Time: Avg: ${serverStats.avgProcessingTime.toFixed(2)}ms`);
        } else {
            console.log("No measurements for this server yet");
        }
        console.log("-------------------------------------\n");
    };

    // Add reset latency statistics command
    const resetLatencyStats = async () => {
        await page.evaluate(() => {
            window.latencyTracker.reset();
        });
        console.log("Latency statistics have been reset.");
    };
    ...
    rl.on('line', async (input) => {
        ...
        if (parts[0] === 'pause') {
            ...
            if (parts.length > 1 && Object.values(SERVER_NAMES).includes(parts[1])) {
                await showServerStatus(parts[1]);
            } else {
                await showStatus();
            }
        } else if (parts[0] === 'resetlatency') {
            ...
            console.log("\n--- Available Commands ---");
            console.log("pause - Pause the bot");
            console.log("resume - Resume the bot");
            console.log("status - Show overall bot status and latency statistics");
            console.log("status [server] - Show latency statistics for a specific server");
            console.log("resetlatency - Reset latency statistics");
            console.log("exit - Shut down the bot");
